...
| **Imported-union matching (G3-family, NEW for Stage 3)** — consumer-side `match` over an imported union's variants does not bind (`SEM001.unknown_variant`); the kernel is the only place the `Effectus` variants can be matched | `fix:sem001` | Kernel-owned accessor `effectus_identitas` (U1) is the only matcher; consumers read the effect key through it, never matching the imported union themselves; removal = grep-replace after the radix fix | radix lane (Mind routes) |
